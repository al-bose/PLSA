...
def EStep():
    for i in range(0, N):
        for j in range(0, M):
            denominator = float(0)
            for k in range(0, K):
                denominator += beta[k, j] * theta[i, k]
            for l in range(0, K):
                p[i, j, l] = float(beta[l, j] * theta[i,l]) / denominator
...
def MStep():
    for k in range(0, K):
        ...
        for i in range(0, M):
            for j in range(0, N):
                denominator += p[j, i, k] * X[j, i]
    
        for i in range(0, M):
            numerator = float(0)
            for j in range(0, N):
                numerator += p[j, i, k] * X[j, i]
            beta[k, i] = float(numerator) / denominator
        
    for i in range(0, N):
        for j in range(0, K):
            numerator = float(0)
            for k in range(0, M):
                numerator += p[i, k, j] * X[i,k]
            theta[i, j] = float(numerator) / N
...
def LogLikelihood():
    ...
    for i in range(0, N):
        for j in range(0, M):
            theta_beta = float(0)
            for k in range(0, K):
                theta_beta += theta[i, k] * beta[k, j]
            loglikelihood += X[i, j] * math.log(theta_beta)
